...
FIX #9  — All initial_state field names match SessionState exactly.
...
async def send_message(request: MessageRequest):
    ...
    # ------------------------------------------------------------------
    # FIX #10 — Load prior messages from DB (survives server restarts)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Build initial state — only fields that exist in SessionState
    # (started_at removed — not in SessionState TypedDict)
    # ------------------------------------------------------------------
    initial_state: SessionState = {
        ...
        # FIX #10 — carry prior messages forward
        "messages": prior_messages,

        # Memory (populated by load_memory node)
        "customer_facts":      {},
        "dynamic_context":     [],
        "session_summaries":   [],
        ...
        # Token tracking (seeded from prior messages)
    ...
    try:
        ...
        # FIX #10 — persist updated messages back to DB after every turn
