...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const initializeSession = async () => {
      ...
      try {
        // Set customer ID for chat
        setCustomerId(customerId || userId);

        // Start a new chat session
        const sessionResponse = await axios.post(
          `${import.meta.env.VITE_BASE_URL}/chat/start`,
          {
        ...
        setUserData(profileResponse.data.user || profileResponse.data);
      } catch (err) {
        console.error("Failed to initialize session or fetch user data:", err);
        setError("Failed to load session. Please login again.");
  ...
  const handleSend = async () => {
    if (!input.trim() || !sessionId || !customerId) {
      return;
    }

    // Add user message to chat immediately
    const userMessageId = messageIdRef.current++;
    const userMessage = { id: userMessageId, role: "user", text: input };
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const agentMessage = {
          id: agentMessageId,
          role: "agent",
          text: response.data.agent_response || "I couldn't process that. Please try again.",
        };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 text-emerald-400">
        ...
        <p className="text-xl font-semibold">Initializing Session...</p>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden max-w-7xl mx-auto w-full shadow-xl rounded-2xl bg-white/80 mt-4 mb-4 md:mb-8 md:mt-8">
        ...
        <section className="flex flex-col w-full md:w-[70%] h-[60vh] md:h-auto bg-white border-b md:border-b-0 md:border-r border-slate-200 transition-all duration-300">
          ...
          <div className="sticky bottom-0 bg-white/95 border-t border-slate-200 px-3 md:px-8 py-3 md:py-4 flex items-center gap-2 md:gap-3 z-10">
            <input
              type="text"
              className="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-slate-900 shadow-sm placeholder-slate-400 text-base md:text-lg disabled:opacity-50"
            ...
            <button
              ...
              className="p-2 md:p-3 bg-gradient-to-br from-blue-600 to-blue-500 text-white rounded-full hover:from-blue-700 hover:to-blue-600 transition shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 disabled:cursor-not-allowed"
              aria-label="Send"
            >
              {sending ? (
